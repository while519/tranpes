...
for datatype in ['train']:
    f = open('freebase_mtr100_mte100-%s.txt' % datatype, mode='rt')
    lines = f.readlines()
    f.close()
    hlist = []
    llist = []
...
f = open('xdata/entity2idx.pkl', 'wb')
g = open('xdata/idx2entity.pkl', 'wb')
...
for datatype in ['train', 'valid', 'test']:
    ...
    f = open('xdata/FB15k-%s-hs.pkl' % datatype, 'wb')
    g = open('xdata/FB15k-%s-ls.pkl' % datatype, 'wb')
    h = open('xdata/FB15k-%s-ts.pkl' % datatype, 'wb')
